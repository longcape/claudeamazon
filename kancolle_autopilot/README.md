...
- 進撃・補給・入渠の画面操作。サンドボックスでは直接呼んでいる
- `use_buckets` の禁止。入渠タスクがまだ無いので受け皿がない
...
32. **`use_buckets` の禁止は受け皿が無い**。入渠タスクが未実装のため、
    語彙としては受け付けるが何も起きない。
33. **進撃はタスク化していない**。`SandboxSession.run_and_resolve` が
    出撃タスクの成功後に `fight_through` を呼ぶ形で、画面操作としては
    実装されていない。判断（`decide_advance`）は AI Core 側にある。
...
1. **進撃・補給・入渠の画面操作**。判断は AI Core にあるが、操作としては
   サンドボックスを直接呼んでいる。
2. **Discord bot 本体**。コマンドの入口は標準入力のみ。
