  ...
  phases: [                      // 局面。最大 4 枚（board.js の MAX_PHASES）
    {
      ...
      view                       // 表示範囲 { zoom, cx, cy }。全体表示なら持たない
    }
  ]
}

round = {
  n, side, tacticId, economy, result: 'WIN'|'LOSS', note, at,
  exec: 'clean'|'partial'|'failed'|'unrated',   // 作戦どおりに実行できたか
  reasons: [...],                               // 成否の理由（15 種・複数可）
  reasonNote                                    // その他の補足（120 字）
}
```
...
### 盤面の拡大は viewBox を切り取る。座標は書き換えない

局所ズーム（A ラッシュなら A 側だけ見る）は、`phase.view = { zoom, cx, cy }` を持たせて
SVG の `viewBox` を切り取ることで実現している。マークもルートも同じ 0-100 空間に載っているので、
**下地と一緒に拡大・移動し、保存されている座標は一切変わらない。**
「拡大したぶんマークの x, y を計算し直す」という作りにしないこと。必ずズレる。

- 倍率は 1〜4 倍（`MIN_ZOOM` / `MAX_ZOOM`）。中心は `clampView()` が盤面の外を映さない範囲へ丸める。
- **全体表示（zoom = 1）のときは `view` キー自体を持たない。** こうしておくと、ズームを使っていない
  戦術の保存内容が以前とまったく同じ形のままになる（古いデータとの差分が出ない）。
- ポインタ座標の変換（`toBoardPoint`）は**必ず viewBox を通す**。画面の比率だけで 0-100 に直すと、
  拡大中に置いたものが必ずズレる。パンの移動量も `toBoardDelta` で同じ換算をする。
- パンは「拡大中に、マークの無いところを掴んだとき」だけ。全体表示では動かす先が無いので
  何も起きない＝**既存の置く・選ぶ・消すは、拡大していない限り一切変わらない**。
- 2 本指のピンチは `pointerdown` の本数で通常操作と切り分ける。
  `Map.values()` はイテレータなので `Array.prototype.slice` では配列にならない。
  ここを取り違えて、**2 本目の指が触れた瞬間に落ちる**バグを実際に出した（`Array.from` を使う）。

### 推奨度の数値は「成功確率」ではない

ライブ画面のカードに出る 0-100 は **ルールベースの推奨度（いまの状況への合い方）**であって、
統計モデルでも学習でもなく、成功確率でも勝率の予測でもない。

- 計算は `advisor.js`。基準点 50（`BASE_SCORE`）に `WEIGHTS` の固定値を足し引きして 0-100 に丸めるだけ。
  **同じ状態なら必ず同じ数値になる。**
- 色は `TONE_GOOD = 66` 以上が緑、`TONE_BAD = 38` 以下が赤、間が黄。
  緑＝いまの状況に合っている / 黄＝ふつう / 赤＝いまは優先度が低い。**成功率の色ではない。**
- 足し引きは必ず `apply()` を通し、実際の値を `reasons[].delta` に残す。
  画面の内訳表示とホバーの合計はこれをそのまま出しているので、
  **重みを変えると表示も自動で追従する**（別々に書かない）。smoke-test が
  「内訳の合計＝表示している数値」を見張っている。
- 名前を確率に見える語へ変えない。英語見出しは `SCORE` だと確率と誤読されるので `FIT` にした。
- **ラウンド評価（exec / reasons）をスコアへ混ぜない。** サンプル数が無い段階で混ぜると、
  根拠のない数字に見た目だけの説得力が付く。混ぜていないことを smoke-test で固定してある。
  将来入れるなら、件数の下限と「推定であること」の明示をセットにする。

### ラウンドの評価（作戦品質と遂行品質を分ける）

勝敗だけでは「作戦が悪かった」のか「作戦は良かったが実行が崩れた」のか分けられないので、
ラウンド記録に `exec`（`clean` / `partial` / `failed` / `unrated`）、`reasons[]`（15 種）、
`reasonNote` を持たせている。

- **試合を止めない。** 勝敗を押した時点でラウンドは確定し、評価は後からでよい。
  ライブ画面のバーは 1 タップで遂行度だけ記録でき、押さずに次へ進める（未評価のまま残るだけ）。
  ラウンド履歴のボタンから後で足せる。**確認ダイアログを挟まない。**
- 古い保存データは持っていないので、読み込み時に `unrated` / `[]` / `''` になる（`normalizeRound`）。
- `undo` するとラウンドごと消えるため評価も一緒に消える。入れ直したラウンドは未評価から始まる。
- `statsFor()` が `byExec` / `cleanWinRate` / `brokenWinRate` / `ratedUsed` を返す。
  評価が 1 件も無い戦術には内訳を出さない（0% と誤読させないため）。

### Supabase の自動停止は GitHub Actions で防いでいる

無料プランは 1 週間ほどアクセスが無いと自動で止まり、コミュニティ・ログイン・クラウド保存が
全部使えなくなる（2026-09-11 と 2026-09-21 に実際に止まっていた）。
`.github/workflows/keepalive.yml` が毎日 06:17（日本時間）に、公開キーで投稿一覧を 1 件だけ読む。
**読むだけで何も書かない。** 人がログインする必要は無い。

- 止まってしまった後は、この実行は失敗して GitHub からメールが来るだけで、自動では再開しない
  （再開には管理用の強いトークンが要るので置いていない）。その場合は管理画面か MCP の
  `restore_project` で再開する。
- 公開リポジトリの定期実行は、60 日間コミットが無いと GitHub に止められる。
  毎回ワークフローの有効化を API で掛け直しているが、止められたときは Actions 画面で有効化し直す。
- 接続情報は `pages.yml` と同じ Variables（`SUPABASE_URL` / `SUPABASE_ANON_KEY`）。
  **service role キーはここにも置かない。**

### 保存されていることを画面に出す

入力は `store.save()` で自動保存されるが、**それが分かるのは初回の案内だけ**で、閉じると二度と出ない。
「保存されているのか分からない」はテストで実際に出たつまずきどころなので、上部に状態を常設している。

- `save()` は結果を覚えて `document` へ `vct:saved` を投げる。**store は UI を知らない**ので、
  描画は受け取った側（`app.js` の `bindSaveState`）に任せる。保存を増やすときもこの形を崩さない。
- 保存に失敗した場合（プライベートモード・容量超過）は赤で「保存できていません」を出す。
  黙って落とすと利用者は気づけない。

### 投稿には配置盤も乗せる

文字だけを共有しても「どこに何を置くか」は伝わらない。`tactic_posts.board jsonb` に
`{ phases: [...] }` を入れて、取り込み時に盤面ごと復元する。

- **`board` が無い古い投稿がある。** 取り込み側は必ず「無ければ従来どおり文字だけ」で動くこと。
- **他人の投稿に入っている id を持ち込まない。** 取り込み時に `id` を外し、store 側で採番し直す。
- 匿名でも投稿できるので、大きさの上限は**サーバ側でも**持つ。`CHECK` では `pg_column_size` を使えないため、
  レート制限のトリガの中で見て `BOARD_TOO_LARGE` を投げる。画面側は `friendlyError()` で文言に直す。

### 狭い画面では盤面を先に出す

1180px 以下で縦 1 列になると、一覧 → 盤面 の順だと盤面までスクロールが長い。
`.board-stage { order: -1 }` で盤面を先に出し、スキルのタイルは 4 列に詰めている。
**一覧と盤面の重なりを見ているテストは、順序ではなく矩形の交差で判定すること**
（順序を前提にすると、並びを変えただけで落ちる）。
...
配置盤の下の案内文（`.board-hint`）は、行数が変わると盤面の大きさの計算が狂うので
「折り返さない 1 行」にしてある。これに幅の上限が無いと、**長い文言がそのまま横幅になり、
グリッドの列 → カード の順に押し広げて画面からはみ出す。** 実際に PC 向けの長い案内文で、
幅 375px の画面に 569px のカードが出ていた。

グリッドやフレックスの子は既定で「中身より縮まない」（`min-width: auto`）。
`.modal-card` と `.board-stage` に `min-width: 0`、狭い画面の `.board-layout` は
`minmax(0, 1fr)`、案内文には `max-width: 100%` を入れてある。どれかを外すと崩れる。

もう 1 つ、**スクロールする箱を狭い画面で縦に積むと、行が 0px に潰される。**
配置盤の左右の一覧（`.board-palette`）は PC では自前でスクロールする箱で、スクロールする箱は
「中身より縮んでよい」扱いになる。幅 1180px 以下で縦 1 列に積んだとき、カードの高さが足りないと
一覧の行が 0px まで潰され、中身だけが盤面の上へはみ出してボタンが押せなくなっていた。
縦 1 列のときは `overflow: visible` にして、外側の `.modal-body` にスクロールを任せている。
この崩れはエージェントを選んだ後（一覧にボタンが並んでから）でないと見えない。

スマホ向けの案内文は短いので、**タッチ端末の条件だけで確かめるとこの崩れは見えない。**
smoke-test は「タッチではない細い画面」でも配置盤が収まることを見ている。
