...
| 進捗（完了 / 作業中 / 未着手 / 次にやること） | `docs/STATUS.md` |
...
```
CLAUDE.md と docs/STATUS.md を読んで、このプロジェクトの現状と残っている作業を教えて
```
...
PC で作業したくない場合は **GitHub Actions** でも同じことができる。
GitHub の Actions タブ → 「公式画像の取得」→ Run workflow を押すだけで、
取得から配布ファイルの作り直しまで済んでコミットされる。
（クラウド上の Claude Code からだけは回線が塞がれていて取得できない。）
