...
| 基準点 | **競技向け改善版（2026-09-12）が現在の正式基準点**。テスト開始時点は `9252e7b`、フォームの反映は `44230e1`、見本投稿の記録は `d9aef3a`。最新は `git log -1` で見る |
| **未コミット変更** | **なし**（リモートと一致） |
| テスト | `node tools/smoke-test.mjs` → **426 件 ok / 0 件 NG**（2026-09-12） |
...
### 競技向けの手直し 4 件 — **2026-09-12 実装・検証済み（現在の正式基準点）**

テスト期間中だが、**競技用途として明らかに価値が高い**と判断して入れた 4 件。
ユーザーの明示承認のうえで本番へ反映した。

- **X 投稿導線を格下げした。** 決着バナーから X ボタンを外し、上部の `⋯` メニューへ
  「X にポスト（試合結果）」として移した。ラウンドの記録が 0 件のときは押せない。
  機能（Web Intent を開く）は従来どおり。**理由は「SNS へ投稿するツール」に見えないようにするため。**
- **配置盤に局所ズームを入れた。** 倍率 1〜4 倍。SVG の `viewBox` を切り取る方式なので、
  置いたエージェント・スキル・ルート・スパイクは下地と一緒に拡大・移動し、**保存座標は書き換わらない**。
  PC はホイール（カーソル位置が軸）＋ ＋/−/全体表示ボタン＋拡大中の余白ドラッグでパン。
  スマホは ＋/− /全体表示＋ 1 本指パン＋ 2 本指ピンチ。ライブ画面の配置盤にも同じ操作がある
  （編集画面で寄せた範囲がそのまま出るので、戻せないと詰むため）。
  倍率と位置は**局面ごと**に保存。全体表示のときは `view` キー自体を持たないので、
  ズームを使っていない戦術の保存内容は以前とバイト単位で同じ形のまま。
- **ラウンドの成否理由を記録できるようにした。** 勝敗に加えて
  `exec`（実行できた / 一部崩れた / 実行できなかった / 未評価）、`reasons[]`（15 種・複数選択）、
  `reasonNote`（その他の補足 120 字）。ライブ画面の評価バーで **1 タップ**、詳細はモーダル、
  ラウンド履歴から後から編集できる。**試合を止めないよう入力はすべて任意。**
  集計は `statsFor()` が `byExec` / `cleanWinRate` / `brokenWinRate` / `ratedUsed` を返し、
  戦術別成績に「作戦通り ○%／崩れた ○%」を出す。
- **カードの数値（推奨度）を監査して、意味を画面に明示した。** 分類は
  **ルールベースの推奨度**で、成功確率でも勝率の予測でもない（詳細は `CLAUDE.md`）。
  重みを `advisor.js` の `WEIGHTS` に集約し、閾値を `TONE_GOOD=66` / `TONE_BAD=38` として公開。
  カードには加点・減点の内訳を出し、ホバーで基準点と合計を読める。説明行で
  「勝率や成功確率ではない」「緑＝合っている / 黄＝ふつう / 赤＝いまは優先度が低い」を明記。
  英語の見出しは `SCORE` → `FIT` に変更（確率と誤読されるため）。

**このとき見つけて直した不具合**: 2 本指ピンチの座標取得で `Map.values()` に
`Array.prototype.slice` を使っており、**2 本目の指が触れた瞬間に必ず例外**になっていた。
`Array.from` へ修正済み（回帰テストあり）。

### Supabase の自動停止を防ぐ — **2026-09-21**

`.github/workflows/keepalive.yml`（Actions「Supabase の自動停止を防ぐ」）が毎日 06:17（日本時間）に
投稿一覧を 1 件だけ読みに行く。読むだけで書き込まない。止まっていたら実行が失敗してメールが届く。
仕組みと注意点は `CLAUDE.md` の「Supabase の自動停止は GitHub Actions で防いでいる」。

### テストで出たつまずきどころ 10 件の手当て — **2026-09-21**

ユーザーテスト中に記録していた「分かりにくい」10 件を、まとめて直した（本人の指示で凍結を解除）。

| 直したもの | 中身 |
| --- | --- |
| 保存が分からない | 上部に**保存の状態を常設**（「保存済み 12:34」／保存できなければ赤で警告）。押すと保存先の説明が出る。`store.save()` が `vct:saved` を投げ、`ui.renderSaveState()` が受ける |
| 配置盤を開いたら一覧が空 | エージェント未設定のとき、パレットに**「エージェントを選びに行く」**ボタン。押すと配置盤を閉じて構成パネルへ送り、一瞬光らせる |
| 取り込んでも配置が付かない | **投稿に配置盤を含めた**（`tactic_posts.board jsonb` を追加）。取り込みで盤面ごと復元する。他人の id は持ち込まない。`board` が無い古い投稿は従来どおり |
| 拡大と表示サイズの区別 | 名前を「マップ拡大」「盤面の大きさ」に変更（3 言語） |
| スマホのタブが語中で折れる | 560px 以下で字間と余白を詰め、`white-space: nowrap`。400px 以下では番号を隠す |
| モーダル見出しが折れる | 560px 以下で `.modal-head` を折り返し可にし、見出しは `nowrap`、副題は 1 行占有して省略しない |
| 見出し番号がタブと不一致 | 画面ごとに 01 から振り直す（ライブ 01/02、コミュニティ 01） |
| スマホでマッチ開始まで遠い | 820px 以下で `.setup-footer` を `position: sticky; bottom: 0`。**スクロールせずに押せる** |
| スマホの配置盤が縦に長い | 1180px 以下で盤面を先に出し（`.board-stage { order: -1 }`）、スキルのタイルを 4 列に詰める |
| トーストがボタンに重なる | モーダルが開いているときは `toast.is-high` で上へ逃がす（判定は `toast()` の中） |

**DB の変更（本番へ適用済み）**: `tactic_posts` に `board jsonb`（null 可）を追加し、
レート制限トリガに `pg_column_size(new.board) > 65536` の上限を足した（`BOARD_TOO_LARGE`）。
既存の行と既存の投稿画面はそのまま動く。戻すときは `alter table public.tactic_posts drop column board;`。

### 戦術カードのディシジョンツリー（DAG）化 — **設計のみ完了。実装は未着手**

設計・監査の正本はエージェントホーム側の
`04_アプリ開発/戦術カード_ディシジョンツリー化_設計監査_2026-09-12.md`。要点:

- 現行の `tactic.next.win/loss` は「エッジ 2 本・条件は勝敗のみ」という提案モデルの**特殊形**で、
  カード側は無改造のままノードとして使える。
- 推奨は**循環を許す有向グラフ**（純 Tree だと再利用ができず、循環禁止 DAG だと自己ループが作れない）。
- データは **DB テーブル化せず**、state に `edges` / `playbooks` を足す。
  ただし `hydrate()` はキーを明示コピーするので、**足し忘れると export/import で消える**。
- 保存形式の `version` は**上げない**（`load()` が `version !== 1` を丸ごと拒否するため）。
- **結論: いま Phase 1 へ進まない。** 受け皿だけの Phase 0 →ユーザーテストの結果 → Phase 1 の順。
