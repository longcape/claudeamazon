...
### はじめてのユーザーテストに向けた手直し — **2026-09-11**

公開ページを「初めて開いた人」の状態（保存データなし）で見直し、使えなくなるものだけを直した。
テストの手順と見るところは `docs/USER-TEST.md`。

- **何のアプリか・何から押すか・どこに保存されるかが、画面のどこにも書かれていなかった。**
  セットアップ画面のいちばん上に案内を出す。「わかった」で閉じると次からは出ない。
  保存はこのブラウザに自動で行われ、ログインは要らないことを明記した。
  クラウド保存の一文は、接続情報が無い版（ボタン自体が無い）では出さない
- **投稿すると味方と相手のエージェント構成まで公開されることが、投稿画面に書かれていなかった。**
  プレビューには出ないので、公開される項目と、公開されないもの（チーム名・マッチメモ）を書いた
- **狭い画面で配置盤の編集画面が横にはみ出していた。** 盤面の下の案内文が「折り返さない 1 行」で、
  PC 向けの長い文言がカードごと押し広げていた。グリッドとカードに `min-width: 0`、
  案内文に `max-width: 100%` を入れ、盤面の大きさも横幅で抑えるようにした
- **幅 1180px 以下（タブレット・スマホ）で、配置盤のエージェントとスキルの一覧が盤面に重なっていた。**
  以前からあった崩れ。縦 1 列に並べたとき、一覧が自前でスクロールする箱だったため
  「中身より縮んでよい」扱いになり、行の高さが 0px まで潰されて中身だけが盤面の上へはみ出していた。
  画面内に持ってきた一覧のボタン 25 個のうち 19〜23 個が別の要素に隠れて押せなかった（修正後は 0 個）。
  縦 1 列のときは一覧を自前でスクロールさせず、外側のモーダルがまとめてスクロールするようにした
- **スマホに右クリックは無いのに、消し方を「右クリック」と案内していた。**
  タッチの端末では「ドラッグで配置。タップで選んで削除」に差し替える（タップで選ぶと削除ボタンが出る）

直していないもの（理由つき）は `docs/USER-TEST.md` の「分かっている弱点」。

### フィードバックの受け皿（候補と推奨）

| 方式 | 答える人の手間 | こちらの準備 | 向き不向き |
| --- | --- | --- | --- |
| **Google フォーム（推奨）** | アカウント不要。スマホで答えられる | Google アカウントでフォームを作る（5 分ほど） | 選択式と自由記述を混ぜられ、回答が表にまとまる |
| GitHub Issues | GitHub のアカウントが要る | 不要（すでにある） | VALORANT のプレイヤーには敷居が高い。書いた内容が公開される |
| メールのリンク | メールアプリが要る | 不要 | 形式がばらばらになる。受け取るアドレスが公開される |
| アプリ内のフォーム（Supabase に保存） | いちばん少ない | 新しい機能になる。いたずら対策も要る | 今回は見送り |
| Discord | 相手とのつながり次第 | サーバの用意 | 雑談に混ざって拾いにくい |

**推奨は Google フォーム。** 新しいサービスへの登録は要らないが、作るのは運営者のアカウントなので、
こちらでは作っていない。聞く項目は `docs/USER-TEST.md` の「第 3 段階」をそのまま使える。

**2026-09-11 にリンクの受け口を入れた。** ページのいちばん下（フッター）に
「ご意見・不具合の報告」を置き、新しいタブで開く（`rel="noopener noreferrer"`）。
リンク先は `assets/js/config.js` の `FEEDBACK_URL` で、**空のあいだはリンクごと隠れる。**
`https://` で始まらない値（打ち間違いや `javascript:`）も出さない。
初見の行動を歪めないよう、最初の案内にはフォームへの誘導を書いていない。

**テスト期間中は原則として機能を足さない。** 例外と記録の仕方は `docs/USER-TEST.md` の
「テスト期間中の決まりごと」。
