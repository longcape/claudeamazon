...
| 最新コミット | `65969de` サンドボックスで死んでいた確認ダイアログをアプリ内モーダルに置き換える |
| **未コミット変更** | **なし**（リモートと一致） |
...
| README の記述ずれの解消 | X ポストの位置など、実装より古い記述が数か所残っている |
...
2. **README の記述ずれを直す**（X ポストの位置、`abilities.js` の「同時展開数」の記述など）
3. BUY マネー計算 — ユーザーが再開を決めたら着手
4. 公開先の常設化を検討する（GitHub Pages 等。現在の URL はクラウドセッション発行で、ローカルから更新できない）
