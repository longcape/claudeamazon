...
function credentials() {
  ...
  return { appId: appId, affiliateId: process.env.RAKUTEN_AFFILIATE_ID || '' };
}
...
async function execute(url) {
  for (let attempt = 0; attempt <= MAX_RETRY; attempt += 1) {
    ...
    let res;
    try {
      res = await fetch(url, { headers: { 'User-Agent': 'rakuten-room-engine/1.0' } });
    } catch (e) {
...
function call(endpoint, params) {
  ...
  const run = chain.then(function () { return execute(url); });
  /* 失敗しても後続を止めない */
